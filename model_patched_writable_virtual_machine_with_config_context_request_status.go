...
// PatchedWritableVirtualMachineWithConfigContextRequestStatus * `offline` - Offline * `active` - Active * `planned` - Planned * `staged` - Staged * `failed` - Failed * `decommissioning` - Decommissioning * `maintenance` - Maintenance * `testing` - Testing
...
const (
	...
	PATCHEDWRITABLEVIRTUALMACHINEWITHCONFIGCONTEXTREQUESTSTATUS_MAINTENANCE     PatchedWritableVirtualMachineWithConfigContextRequestStatus = "maintenance"
	PATCHEDWRITABLEVIRTUALMACHINEWITHCONFIGCONTEXTREQUESTSTATUS_TESTING         PatchedWritableVirtualMachineWithConfigContextRequestStatus = "testing"
)
